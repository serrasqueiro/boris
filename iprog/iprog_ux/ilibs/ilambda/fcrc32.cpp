...
int fCRC32::ComputeCRC32 (const char* strName, t_uint32& outCrc32)
{
 lastOpError = 0;

 if ( strName ) {
     outCrc32 = (t_uint32)bsdFileCRC( strName );
 }
...
unsigned long fCRC32::UpdateCRC (unsigned long CRC, const char *buffer, long count)
{
 /*
...
unsigned long fCRC32::bsdFileCRC (const char* filename)
{
...
#define BUFFERSIZE 65536*16
 ...
 if (fIn != NULL) {
     do {
	 if ((localread = fread(buffer, 1, BUFFERSIZE, fIn))) {
...
////////////////////////////////////////////////////////////
int hex_char (char chr)
{
 if ( chr>='0' && chr<='9' ) return chr-'0';
 if ( chr>='a' && chr<='f' ) chr -= 32;
 if ( chr>='A' && chr<='F' )
     return (chr - 'A') + 10;
 return -1;
}
